...
describe('InvestmentForm', () => {
  ...
  it('debe renderizar todos los campos del formulario', () => {
    renderWithLanguage(<InvestmentForm {...defaultProps} />);
  ...
  it('debe llamar onInputChange al cambiar la inversión inicial', async () => {
    ...
    renderWithLanguage(<InvestmentForm {...defaultProps} onInputChange={onInputChange} />);
  ...
  it('renderiza el campo de frecuencia de capitalización con valor inicial', () => {
    const onInputChange = jest.fn();
    renderWithLanguage(<InvestmentForm {...defaultProps} onInputChange={onInputChange} />);
    ...
    // Valor por defecto es monthly -> "Mensual" en español
    expect(screen.getByRole('combobox')).toHaveTextContent(/mensual/i);
  });

  it('debe llamar onCalculate al enviar el formulario', async () => {
    const user = userEvent.setup();
    const onCalculate = jest.fn();
    renderWithLanguage(<InvestmentForm {...defaultProps} onCalculate={onCalculate} />);
  ...
  it('debe llamar onReset al hacer clic en Limpiar', async () => {
    ...
    renderWithLanguage(<InvestmentForm {...defaultProps} onReset={onReset} />);
  ...
  it('debe mostrar errores de validación', () => {
    ...
    renderWithLanguage(<InvestmentForm {...defaultProps} errors={errors} />);
  ...
  it('debe mostrar indicador de carga cuando isCalculating es true', () => {
    renderWithLanguage(<InvestmentForm {...defaultProps} isCalculating={true} />);
  ...
  it('debe calcular correctamente años y meses para 125 meses', () => {
    ...
    renderWithLanguage(<InvestmentForm {...defaultProps} inputs={inputs} />);

    // 125 meses = 10 años y 5 meses
    expect(screen.getByText(/10 años y 5 meses/i)).toBeInTheDocument();
  });

  it('debe tener el campo de frecuencia de capitalización con valor inicial', () => {
    renderWithLanguage(<InvestmentForm {...defaultProps} />);
